...
services_seeds.each do |name|
  RecurringService.create(name: name)
end

client_price_seeds.each do |name, price, id|
  RecurringPrice.create(name: name, price: price, client_id: id)
end

invoice_seeds.eachd do |date, performed, status, notes, total, client_id|
  Invoice.create(date: date, performed_by: performed, status: status, notes: notes, total: total, client_id: client_id)
end